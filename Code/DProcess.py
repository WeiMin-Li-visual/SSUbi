...
def Structure_onehot(sampleSeq3DArr):  # 将CEHB组成的结构特征进行独热编码
    letterDict = {}
    letterDict["C"] = 0
    letterDict["E"] = 1
    letterDict["H"] = 2
...
def convertRawToXY(rawDataFrame, refMatrFileName="", nb_windows=3, codingMode=0):  # rawDataFrame is numpy.ndarray
    ...
    if codingMode == 0:
        ...
        probMatr = Structure(sampleSeq3DArr)
    elif codingMode == 6:
        probMatr = Structure_onehot(sampleSeq3DArr)
...
def CKSAAP(fastas, gap=5):
    ...
    label = encodings[:, 0]  # 取所有数组的第0个数据
    data = encodings[:, 1:]  # 取所有数组的第1-最后 的数据
